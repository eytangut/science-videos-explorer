...
export default function VideoList({ videos, onMarkAsWatched, watchedVideoIds, isLoading, error, hasChannels }: VideoListProps) {
  if (isLoading) {
    return (
  ...
  if (!hasChannels) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-6 text-center">
        ...
        <p className="text-muted-foreground">Add some YouTube channels using their RSS feeds to see videos here.</p>
      </div>
    );
  }
  
  if (unWatchedVideos.length === 0 && hasChannels) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-6 text-center">
        <AlertCircle className="h-16 w-16 text-muted-foreground mb-4" />
        <h2 className="text-2xl font-headline mb-2">All Caught Up!</h2>
        <p className="text-muted-foreground">No new videos to display, or all videos have been marked as watched.</p>
      </div>
    );
  }
    ...
    <ScrollArea className="h-full">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-8 p-6">
        {unWatchedVideos.map(video => (
          <VideoCard
            key={`${video.channelRssUrl}-${video.id}`}
            video={video}
